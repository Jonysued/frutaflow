...
export default function ImportModal({ entity, onClose }) {
  ...
  const handleImport = async () => {
    ...
    if (records.length === 0) {
      setStatus("error");
      setMessage("No se encontraron registros válidos.");
      return;
    }
    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-md">
        ...
        <div className="p-5 space-y-4">
          <p className="text-xs text-gray-500">Subí un archivo <strong>.xlsx</strong> o <strong>.csv</strong> con los datos.</p>
          <div className="border-2 border-dashed border-gray-200 rounded-xl p-6 text-center cursor-pointer hover:border-[#c0392b] transition-colors" onClick={() => inputRef.current?.click()}>
          ...
          {status === "success" && <div className="flex items-center gap-2 bg-green-50 text-green-700 rounded-lg px-3 py-2 text-xs"><CheckCircle className="w-4 h-4" /> {message}</div>}
          {status === "error" && <div className="flex items-center gap-2 bg-red-50 text-red-700 rounded-lg px-3 py-2 text-xs"><AlertCircle className="w-4 h-4" /> {message}</div>}
        </div>
        <div className="px-5 py-4 border-t flex gap-2 justify-end">
          <button onClick={onClose} className="px-4 py-2 border rounded-lg text-xs text-gray-600 hover:bg-gray-50">{status === "success" ? "Cerrar" : "Cancelar"}</button>
          {status !== "success" && (
            <button onClick={handleImport} disabled={!file || status === "loading"} className="px-4 py-2 bg-[#c0392b] text-white rounded-lg text-xs font-semibold hover:bg-[#a93226] disabled:opacity-60 flex items-center gap-1">
              {status === "loading" ? <><div className="w-3 h-3 border-2 border-white/40 border-t-white rounded-full animate-spin" /> Procesando...</> : <><Upload className="w-3 h-3" /> Importar</>}
            </button>
          )}
        </div>
      </div>
