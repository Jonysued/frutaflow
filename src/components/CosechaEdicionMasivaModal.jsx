...
export default function CosechaEdicionMasivaModal({ onClose, onSaved }) {
  ...
  const handleSave = async () => {
    ...
    const updates = registros
      .filter(r => edits[r.id])
      .map(r => base44.entities.Cosecha.update(r.id, { procedencia: edits[r.id] }));
              ...
              <table className="w-full text-xs">
                <thead className="bg-[#5c1020] text-white">
                  <tr>
                    ...
                    <th className="px-2 py-2 text-left min-w-[160px]">Procedencia ✏️</th>
                  </tr>
                </thead>
                <tbody>
                  {registros.map((r, i) => (
                    <tr key={r.id} className={i % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                      ...
                      <td className="px-2 py-1">
                        <select
                          ...
                          className="w-full border rounded px-1 py-1 text-xs focus:outline-none focus:ring-1 focus:ring-[#c0392b]"
                        >
                          <option value="">-- Seleccionar --</option>
                          {procNombres.map(n => <option key={n} value={n}>{n}</option>)}
                        </select>
                      </td>
                    </tr>
                  ))}
