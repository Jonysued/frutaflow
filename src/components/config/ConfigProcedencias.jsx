...
export default function ConfigProcedencias() {
  ...
  const load = async () => {
    const [procs, props] = await Promise.all([base44.entities.Procedencia.list(), base44.entities.Propietario.list()]);
    setItems(procs);
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <p className="text-xs text-gray-500 mt-0.5">Al elegir propietario en cosecha se filtra la lista de procedencias.</p>
      </div>
          ...
          <tbody>
            {items.map(item => (
              <tr key={item.id} className="border-b">
                ...
                <td className="px-3 py-2"><input value={editMap[item.id]?.nombre || ""} onChange={e => setEdit(item.id, "nombre", e.target.value)} className={inputCls} /></td>
                <td className="px-3 py-2"><input value={editMap[item.id]?.descripcion || ""} onChange={e => setEdit(item.id, "descripcion", e.target.value)} className={inputCls} /></td>
            ...
            <tr className="border-t bg-gray-50">
              ...
              <td className="px-3 py-2"><input value={newRow.nombre} onChange={e => setNewRow(f => ({ ...f, nombre: e.target.value }))} className={inputCls} placeholder="Ej: OP1SE" /></td>
