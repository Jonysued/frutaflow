...
function ProcedenciaRow({ item, onDelete, onUpdate }) {
  const [form, setForm] = useState({ nombre: item.nombre, descripcion: item.descripcion || "" });
  const [saving, setSaving] = useState(false);
  const inputCls = "border rounded-lg px-2 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-[#c0392b] w-full";

  const save = async () => {
    setSaving(true);
    await base44.entities.Procedencia.update(item.id, form);
    setSaving(false);
    onUpdate();
  };
...
export default function ConfigProcedencias() {
  ...
  const load = () => base44.entities.Procedencia.list().then(setItems);
  useEffect(() => { load(); }, []);
  ...
  return (
    <div className="space-y-4">
      <div>
        <h3 className="font-semibold text-gray-800 text-sm">Procedencias</h3>
        <p className="text-xs text-gray-500 mt-0.5">Lista de procedencias disponibles para seleccionar al registrar un BIN de cosecha.</p>
      </div>
      <div className="overflow-x-auto border rounded-xl">
        <table className="w-full text-sm">
          ...
          <tbody>
            {items.map(item => <ProcedenciaRow key={item.id} item={item} onDelete={handleDelete} onUpdate={load} />)}
            <tr className="border-t bg-gray-50">
              <td className="px-3 py-2"><input value={newRow.nombre} onChange={e => setNewRow(f => ({ ...f, nombre: e.target.value }))} className={inputCls} placeholder="Ej: OP1SE, OP2N" /></td>
