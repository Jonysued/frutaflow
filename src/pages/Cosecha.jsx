...
export default function Cosecha() {
  ...
  const load = () => {
    setLoading(true);
    base44.entities.Cosecha.list("-fecha", 200).then(r => { setRegistros(r); setLoading(false); });
  };
  ...
  const downloadTemplate = () => {
    const header = "fecha,turno,variedad,cuartel,cuadrilla,tipo_bulto,cantidad_bultos,kilos_totales,notas";
    const example = "2024-01-15,Mañana,Granada,Cuartel 1,Cuadrilla A,Bin,50,2500,";
    ...
    <div className="p-4 md:p-6 space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500">Registro de bins y bultos cosechados</p>
        </div>
      ...
      {showImport && (
        <ImportModal entity="Cosecha" onClose={() => { setShowImport(false); load(); }} />
      )}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-[#5c1020] text-white text-xs">
                  ...
                  <th className="px-3 py-3 text-left">Cuartel</th>
                  <th className="px-3 py-3 text-left hidden sm:table-cell">Cuadrilla</th>
                  <th className="px-3 py-3 text-left hidden md:table-cell">Tipo bulto</th>
              ...
              <tbody>
                {registros.length === 0 && (
                  <tr><td colSpan={9} className="text-center py-10 text-gray-400">No hay registros aún</td></tr>
                )}
                {registros.map((r, i) => (
                  <tr key={r.id} className={i % 2 === 0 ? "bg-white" : "bg-[#fdf4f5]"}>
                    <td className="px-3 py-2.5 font-medium text-gray-700">{r.fecha}</td>
                    <td className="px-3 py-2.5">
                      <span className="px-2 py-0.5 rounded-full text-[11px] font-semibold bg-[#f8d7da] text-[#7a1a30]">{r.turno}</span>
                    </td>
                    <td className="px-3 py-2.5 text-gray-700">{r.variedad}</td>
                    <td className="px-3 py-2.5 text-gray-600">{r.cuartel}</td>
                    <td className="px-3 py-2.5 text-gray-600 hidden sm:table-cell">{r.cuadrilla}</td>
                    <td className="px-3 py-2.5 text-gray-600 hidden md:table-cell">{r.tipo_bulto}</td>
                    <td className="px-3 py-2.5 text-right font-medium">{r.cantidad_bultos?.toLocaleString()}</td>
                    <td className="px-3 py-2.5 text-right font-bold text-[#5c1020]">{r.kilos_totales?.toLocaleString()}</td>
